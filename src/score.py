...
def mlflow_score(models):
    with mlflow.start_run(run_name='SCORE'):
        ...
        lin_scores,tree_scores,forest_scores,grid_search_scores = scoring(testset,models[0],models[1],models[2],models[3])
        print(lin_scores,tree_scores,forest_scores,grid_search_scores)
        mlflow.log_metrics({'lin_mae':lin_scores[0],'tree_mae':tree_scores[0],'forest_mae':forest_scores[0],'grid_search_mae':grid_search_scores[0]})
        mlflow.log_metrics({'lin_mse':lin_scores[1],'tree_mse':tree_scores[1],'forest_mse':forest_scores[1],'grid_search_mse':grid_search_scores[1]})
        mlflow.log_metrics({'lin_rmse':lin_scores[2],'tree_rmse':tree_scores[2],'forest_rmse':forest_scores[2],'grid_search_rmse':grid_search_scores[2]})
